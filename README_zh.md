...
## 支持的工具（142 个 provider）

vx 通过 Starlark DSL provider 管理 142 个工具。以下是部分亮点：
...
| **Zig** | `zig` | Zig 编程语言 |
| **Python** | `python`（通过 UV） | Python 运行时 |
| **.NET** | `dotnet` | .NET SDK 和运行时 |
...
| **k3d** | `k3d` | 轻量级 Docker 中的 Kubernetes |
| **Skaffold** | `skaffold` | Kubernetes 持续开发 |

### 代码质量和安全

| 工具 | 命令 | 描述 |
|------|----------|-------------|
| **Ruff** | `ruff` | 快速 Python 代码检查和格式化 |
| **Biome** | `biome` | Web 快速格式化和代码检查 |
| **oxlint** | `oxlint` | 氧化的 JavaScript 代码检查 |
| **golangci-lint** | `golangci-lint` | Go 代码检查聚合器 |
| **Trivy** | `trivy` | 安全扫描器 |
| **Grype** | `grype` | 漏洞扫描器 |
...
| **fzf** | `fzf` | 模糊查找器 |
| **jq** | `jq` | JSON 处理器 |
| **yq** | `yq` | YAML/JSON/XML 处理器 |
| **delta** | `delta` | 更好的 git diff |
...
> 运行 `vx list` 查看所有 142 个支持的工具，或使用 `vx search <query>` 搜索特定工具。
...
| **团队同步** | `vx setup` | 手动协调 |
| **MCP 就绪** | 只需添加 `vx` | 复杂设置 |
| **跨平台** | Windows/macOS/Linux | 因工具而异 |
| **AI Agent DX** | 结构化输出、模式自省 | 无 Agent 支持 |
