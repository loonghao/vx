...
AI agent 上下文压缩工具。可在保留语义的同时减少 40-60% 的 token 消耗。
...
将 headroom 作为 MCP 服务器添加到 AI agent 的配置中：
