...
"#;
    ...
    let xcodebuild = manifest.runtimes.iter().find(|r| r.name == "xcodebuild").unwrap();
    assert!(xcodebuild.platform_constraint.is_some());
    assert_eq!(xcodebuild.platform_description(), Some("macOS only".to_string()));
    
    let cross_tool = manifest.runtimes.iter().find(|r| r.name == "cross-platform-tool").unwrap();
