...
use vx_resolver::{Ecosystem, LockFile, LockedTool, ResolvedVersion, Version, VersionRequest, VersionSolver};
...
async fn resolve_tool_version(
    ...
    let is_passthrough = versions
        .iter()
        .any(|v| v.metadata.get("passthrough").map(|s| s == "true").unwrap_or(false));
    ...
    let download_url = if let Ok(Some(url)) = runtime.download_url(&resolved.version.to_string(), &current_platform).await {
        Some(url)
    } else {
        if verbose {
