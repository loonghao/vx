            ...
            for release in releases_array {
                ...
                if is_lts {
                    ...
                    if let Some(lts_name) = lts_info {
                        version_info = version_info.with_metadata("lts_name".to_string(), lts_name.to_string());
                    }
                } else {
                ...
                // Add download URL
                if let Some(download_url) = crate::url_builder::NodeUrlBuilder::download_url(&version_info.version) {
    ...
    pub fn parse_versions(json: &Value, include_prerelease: bool) -> Result<Vec<VersionInfo>> {
        ...
        if let Some(releases_array) = json.as_array() {
            for release in releases_array {
                ...
                if stable {
                    version_info = version_info.with_metadata("stable".to_string(), "true".to_string());
                }
                
                // Add download URL
                if let Some(download_url) = crate::url_builder::GoUrlBuilder::download_url(&version_info.version) {
                    version_info = version_info.with_download_url(download_url);
                }
