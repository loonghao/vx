...
use crate::{VxPlugin, VxTool, VxPackageManager, Result, VxError};
...
impl PluginRegistry {
    ...
    pub fn get_package_manager(&self, pm_name: &str) -> Option<Box<dyn VxPackageManager>> {
        if let Some(plugin) = self.find_plugin_for_package_manager(pm_name) {
            plugin.package_managers().into_iter()
...
mod tests {
    use super::*;
    use crate::{VersionInfo, Ecosystem, ToolContext, ToolExecutionResult, PackageSpec, PackageInfo};
    use std::path::Path;
    ...
    impl VxPackageManager for MockPackageManager {
        ...
        async fn install_packages(&self, _packages: &[PackageSpec], _project_path: &Path) -> Result<()> {
            Ok(())
        }
    }
    ...
    impl VxPlugin for MockPlugin {
        ...
        fn tools(&self) -> Vec<Box<dyn VxTool>> {
            vec![Box::new(MockTool { name: "test-tool".to_string() })]
        }
        
        fn package_managers(&self) -> Vec<Box<dyn VxPackageManager>> {
            vec![Box::new(MockPackageManager { name: "test-pm".to_string() })]
        }
    }
