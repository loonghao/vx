...
impl ProviderEnvironment {
    ...
    pub fn build_env_vars(&self) -> HashMap<String, String> {
        ...
        for (key, value) in &self.manifest_env_vars {
            ...
            let expanded = value
                .replace("{install_dir}", &self.version_info.install_dir.display().to_string())
...
impl VersionResolverCache {
    ...
    pub fn resolve(
        ...
        let install_dir = PathBuf::from(format!("/.vx/store/{}/{}", runtime_name, resolved_version));
...
mod tests {
    ...
    fn test_provider_environment_env_vars() {
        ...
        let env = ProviderEnvironment::new(
            version_info,
            "python".to_string(),
            "python".to_string(),
        );
        ...
        assert_eq!(
            vars.get("VX_PYTHON_VERSION"),
            Some(&"3.11.11".to_string())
        );
        assert_eq!(
    ...
    fn test_provider_environment_manifest_vars_expansion() {
        ...
        manifest_vars.insert("MY_TOOL_ROOT".to_string(), "{install_dir}/tools".to_string());
