...
impl Runtime for RustupRuntime {
    ...
    fn post_extract(&self, _version: &str, install_path: &std::path::PathBuf) -> Result<()> {
        ...
        stdout_handle.join().map_err(|e| anyhow::anyhow!("Stdout thread failed: {:?}", e))?;
        stderr_handle.join().map_err(|e| anyhow::anyhow!("Stderr thread failed: {:?}", e))?;
...
impl Runtime for CargoRuntime {
    ...
    async fn install(&self, version: &str, ctx: &RuntimeContext) -> Result<vx_runtime::InstallResult> {
        ...
        if Self::check_rustup_toolchain_exists(&rustup_exe, version, &rustup_home, &cargo_home) {
            ...
            Self::switch_toolchain(
                &rustup_exe,
                version,
                &rustup_home,
                &cargo_home,
            )?;
            return Ok(InstallResult::success(install_path.clone(), install_path, version.to_string()));
        }

        // Install the toolchain and set as default
        self.post_install(version, ctx).await?;

        Ok(InstallResult::success(install_path.clone(), install_path, version.to_string()))
    }
...
impl Runtime for RustcRuntime {
    ...
    async fn install(&self, version: &str, ctx: &RuntimeContext) -> Result<vx_runtime::InstallResult> {
        ...
        // Check if the toolchain is installed but not default
        if CargoRuntime::check_rustup_toolchain_exists(&rustup_exe, version, &rustup_home, &cargo_home) {
            eprintln!("📦 Switching to Rust toolchain {}...", version);
            CargoRuntime::switch_toolchain(
                &rustup_exe,
                version,
                &rustup_home,
                &cargo_home,
