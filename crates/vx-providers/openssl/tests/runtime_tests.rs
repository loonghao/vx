...
fn test_provider_get_runtime() {
    let provider = create_provider();
    assert!(provider.get_runtime("openssl").is_some());
    assert!(provider.get_runtime("unknown").is_none());
}

#[test]
fn test_star_metadata() {
    let meta = vx_starlark::StarMetadata::parse(vx_provider_openssl::PROVIDER_STAR);
...
fn create_provider() -> std::sync::Arc<dyn vx_runtime::Provider> {
    ...
    vx_starlark::create_provider(name, vx_provider_openssl::PROVIDER_STAR)
}
