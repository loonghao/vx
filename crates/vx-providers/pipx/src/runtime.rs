//! pipx runtime implementation
//!
//! pipx is a tool to help you install and run end-user applications written in Python.
//! It creates isolated environments for each application.
//!
//! This runtime installs pipx from PyPI as an isolated tool via uv.
...
/// pipx runtime implementation
///
/// pipx is installed as a pip package in an isolated virtual environment,
/// using uv for faster installation.
#[derive(Debug, Clone, Default)]
pub struct PipxRuntime;

impl PipxRuntime {
    /// Create a new pipx runtime
    pub fn new() -> Self {
        Self
    }

    /// pip package name for pipx
    const PACKAGE_NAME: &'static str = "pipx";
}

#[async_trait]
impl Runtime for PipxRuntime {
    fn name(&self) -> &str {
        "pipx"
    }
    ...
    fn metadata(&self) -> HashMap<String, String> {
        ...
        meta.insert("homepage".to_string(), "https://pipx.pypa.io/".to_string());
        meta.insert(
            "documentation".to_string(),
            "https://pipx.pypa.io/stable/".to_string(),
        );
        meta.insert(
    ...
    fn executable_relative_path(&self, _version: &str, platform: &Platform) -> String {
        ...
        if platform.is_windows() {
            format!("venv/Scripts/{}", platform.exe_name("pipx"))
        } else {
            format!("venv/bin/{}", platform.exe_name("pipx"))
        }
    }
    ...
    async fn is_installed(&self, version: &str, ctx: &RuntimeContext) -> Result<bool> {
        ...
        let exe_name = if cfg!(windows) { "pipx.exe" } else { "pipx" };
    ...
    fn verify_installation(
        ...
        if exe_path.exists() {
            ...
            VerificationResult::failure(
                vec![format!(
                    "pipx executable not found at expected path: {}",
                    exe_path.display()
                )],
                vec!["Try reinstalling with: vx install pipx".to_string()],
            )
        }
    }
}
...
#[async_trait]
impl PackageRuntime for PipxRuntime {
