...
#[rstest]
#[case(Os::Windows, "shared")]
#[case(Os::Linux, "pgo+lto")]
#[case(Os::MacOS, "pgo+lto")]
fn test_variant(#[case] os: Os, #[case] expected: &str) {
    let platform = Platform {
        os,
        arch: Arch::X86_64,
    };
    assert_eq!(PythonUrlBuilder::get_variant(&platform), expected);
}
...
fn test_download_url_format_python312() {
    ...
    let url = PythonUrlBuilder::download_url_with_date("3.12.8", "20251217", &platform).unwrap();
    ...
    assert!(url.contains("pgo+lto"));
    assert!(url.contains("install_only"));
    assert!(url.ends_with(".tar.gz"));
}

#[test]
fn test_download_url_format_python37() {
    let platform = Platform {
        os: Os::Linux,
        arch: Arch::X86_64,
    };
    // Python 3.7 uses older release dates
    let url = PythonUrlBuilder::download_url_with_date("3.7.17", "20230826", &platform).unwrap();
...
fn test_filename_includes_release_date() {
    ...
    assert!(filename1.contains("20251217"));

    let filename2 =
        PythonUrlBuilder::get_filename_with_date("3.7.17", "20230826", &platform).unwrap();
    assert!(filename2.contains("20230826"));
}

#[rstest]
#[case("3.7.17", "20230826")]
#[case("3.8.20", "20241206")]
#[case("3.9.21", "20251217")]
#[case("3.10.16", "20251217")]
#[case("3.11.11", "20251217")]
#[case("3.12.8", "20251217")]
fn test_download_url_various_versions(#[case] version: &str, #[case] release_date: &str) {
    let platform = Platform {
        os: Os::Linux,
        arch: Arch::X86_64,
    };
