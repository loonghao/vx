...
vx is a **universal development tool manager** written in Rust. Users prefix any command with `vx` (e.g., `vx node --version`, `vx cargo build`) and vx automatically installs, manages, and forwards to the correct tool version. It ships **105 providers** defined via Starlark DSL (`provider.star`).
