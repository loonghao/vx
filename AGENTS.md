...
vx is a **zero-config universal development tool manager** (v0.8.24, MIT-licensed, written in Rust). Users prefix any command with `vx` (e.g., `vx node --version`, `vx cargo build`) and vx automatically installs, manages, and forwards to the correct tool version. vx currently ships **122 providers** covering language runtimes, build tools, DevOps CLIs, cloud platforms, and more — all defined via Starlark DSL (`provider.star`).
...
| See all 122 providers                | [`crates/vx-providers/`](crates/vx-providers/)   |
